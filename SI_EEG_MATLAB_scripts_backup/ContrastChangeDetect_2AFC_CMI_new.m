...
Screen('DrawText', window, 'Press LEFT button with LEFT hand when the LEFT-tilted pattern gets stronger.', 0.05*scresw, 0.30*scresh, 255); %0.05*scresw, 0.35*scresh
Screen('DrawText', window, 'Press RIGHT button with RIGHT hand when the RIGHT-tilted pattern gets stronger.', 0.05*scresw, 0.35*scresh, 255);
...
Screen('DrawText', window, 'First we have to measure the position of your eyes.', 0.05*scresw, 0.45*scresh, 255);
Screen('DrawText', window, 'Just follow with your eyes the the circle:', 0.05*scresw, 0.50*scresh, 255);
Screen('DrawTexture', window, eye_calib, [], [center(1)-10 380 center(1)+10 400] + stimrect_calib);
Screen('DrawText', window, 'Press to begin', 0.05*scresw, 0.75*scresh, 255);
Screen('Flip', window); 
[clicks,x,y,whichButton] = GetClicks(whichScreen,0);
...
if par.useEL, 
    ...
    % stop recording
		...
		ovr = int32(1);
		filename = formatString(256, int8(['C:\PsychToolbox_Experiments\Simon\AA_eyetracker_data\' subj_ID{1,1} '_SAIIT_2AFC' '.idf']));
