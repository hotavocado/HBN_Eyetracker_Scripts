...
for n=1:length(par.contrastsBG)
    % get rid of redundant conditions = for 0% background w don't need to vary spatial phase and orientation!
    nc = par.numconds;
    if par.contrastsBG(n)==0, nc=1; end
    for c=1:nc
...
temp = block;
temp = temp(:,randperm(size(temp,2))); % randomize order
BGcon = temp(1,:);
CNTcon = temp(2,:);
...
if par.useEL, 
    ...
    % save recorded data
		...
		ovr = int32(1);
		filename = formatString(256, int8(['C:\PsychToolbox_Experiments\Simon\AA_eyetracker_data\' subj_ID{1,1} '_SurrSupp' '.idf']));
...
save([par.runID , '_SurroundSupp'],'TargOnT','BGcon','CNTcon','StimCond','RespT','RespLR','par') 
