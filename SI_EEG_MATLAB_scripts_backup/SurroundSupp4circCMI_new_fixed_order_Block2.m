...
BGcon = block(1,size(block,2)/2+1:end);
CNTcon = block(2,size(block,2)/2+1:end);
StimCond = block(3,size(block,2)/2+1:end);   % stimulus condition
...
if par.useEL, eyetr_sendtrigger(par.CD_FIX_ON,sock);end;
    ...
    if par.useEL, eyetr_sendtrigger(par.CD_TG,sock);end;
...
% [success, ivx]=iViewX('datafile', ivx, ['C:\PsychToolbox_Experiments\Simon\AA_eyetracker_data\' subj_ID{1,1} '_SurrSupp_Block2'  '.idf']);
% 
...
save([par.runID , '_SurroundSupp_Block2'],'TargOnT','BGcon','CNTcon','StimCond','RespT','RespLR','par') 
