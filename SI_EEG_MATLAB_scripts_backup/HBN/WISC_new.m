subj_Name = 'Test'
subj_ID =inputdlg({'Enter Subjects ID:', 'Enter Date:','Use EEG:', 'Use Eye-Tracker:', 'Eye-Tracker Calibration:'},'CMI-EEG',1,{subj_Name,date,'y','y', 'y'});
...
filepath_stimuli  = '/home/cmi_linux/PsychToolbox_Experiments/Simon/WISC_PS/single_stimuli/'
filepath_responses = '/home/cmi_linux/PsychToolbox_Experiments/Simon/WISC_PS/responses/'
filepath_eye_calib = '/home/cmi_linux/PsychToolbox_Experiments/Simon/general_matlabfiles/'

files_stimuli = dir('/home/cmi_linux/PsychToolbox_Experiments/Simon/WISC_PS/single_stimuli/*.bmp')
files_responses = dir('/home/cmi_linux/PsychToolbox_Experiments/Simon/WISC_PS/responses/*.bmp')
files_eye_calib = dir('/home/cmi_linux/PsychToolbox_Experiments/Simon/general_matlabfiles/eye_calib.bmp')

filepath_page = '/home/cmi_linux/PsychToolbox_Experiments/Simon/WISC_PS/'
files_page = dir('/home/cmi_linux/PsychToolbox_Experiments/Simon/WISC_PS/next_page.bmp')
...
Screen('DrawText', window, 'Just follow with your eyes the circle:', 0.05*scresw, 0.75*scresh, 0);
...
if par.useDrift == 1 && par.useEL == 1
    ...
    wavfilename_probe1 = '/home/cmi_linux/PsychToolbox_Experiments/Simon/general_matlabfiles/COUNTDOWN_EYETRACKER_DRIFT.wav '
...
if par.activated_resp(pp,resp_b,1) == 2 && par.activated_resp(pp,resp_b,2) == 2 && x(end) >= resp_cross(resp_b,2,1) && x(end) <= resp_cross(resp_b,2,3) && y(end) >= resp_cross(resp_b,2,2) && y(end) <= resp_cross(resp_b,2,4);
            ...
            if par.useEL, [success, ivx]=iViewX('message', ivx,[ num2str(num2str(par.CD_BUTTONS(1)))]);end;
            ...
            vbl = Screen('Flip', window, [],1);
                if vbl >=time+duration %Tests if 120 secs are gone
                           ...
                           % if par.useEL, calllib('iViewXAPI', 'iV_SendImageMessage', formatString(256, int8([ num2str(num2str(par.CD_NXTPG))])));end;
                            ...
                            %stop(timerObj)
                            WaitSecs(2)
                            save([par.runID , '_WISC_ProcSpeed'],'par', 'trial_duration','perf', 'x', 'y', 'whichButton')
                            if par.useEL, 
                                % stop recording   

                            [success, ivx]=iViewX('stoprecording', ivx);
                            [success, ivx]=iViewX('datafile', ivx, ['C:\PsychToolbox_Experiments\Simon\HBN\AA_eyetracker_data\' subj_ID{1,1} '_WISC_ProcSpeed'  '.idf']);
           ...
           % if par.useEL, calllib('iViewXAPI', 'iV_SendImageMessage', formatString(256, int8([ num2str(num2str(par.CD_BUTTONS(1)))])));end;
            ...
            vbl = Screen('Flip', window, [],1);
                if vbl >=time+duration %Tests if 120 secs are gone
                           ...
                           % stop(timerObj)
                            WaitSecs(2)
                            save([par.runID , '_WISC_ProcSpeed'],'par', 'trial_duration', 'perf')
                           if par.useEL, 
                                % stop recording   
